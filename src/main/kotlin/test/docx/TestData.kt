...
class TestData(
    ...
    var inAlternateContent: Boolean = false,
    ...
    companion object {
        ...
        fun createTest(): MutableList<TestData> {
            return resumeServerBeanToThis(Gson().fromJson(TEST_JSON, ResumeServerBean::class.java))
        }

        fun resumeServerBeanToThis(b: ResumeServerBean): MutableList<TestData> {
            return mutableListOf<TestData>().apply {
                add(TestData("name", b.name ?: "", false, inAlternateContent = true))
                add(TestData("gender", b.genderStr, true, inAlternateContent = true))
                add(TestData("age", b.ageStr, true, inAlternateContent = true))
                add(TestData("phoneNumber", b.phone_number ?: "", false, inAlternateContent = true))
                add(TestData("email", b.email ?: "", false, inAlternateContent = true))
                add(TestData("jobExperience", b.jobExperienceStr, true, inAlternateContent = true))
                add(TestData("userAvatar", b.photo ?: "", false, inAlternateContent = true, type = TYPE_IMAGE))
                add(TestData("jobObjective", b.job_objective ?: "", false, inAlternateContent = true))
                add(TestData("expectedIndustry", b.expected_industry ?: "", true, inAlternateContent = true))
                add(TestData("expectedSalary", b.expected_salary ?: "", true, inAlternateContent = true))
            }
        }
    }
